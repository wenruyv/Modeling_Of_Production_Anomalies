...
export default{
  ...
  setup(){
      ...
      try {
        ...
        if (res.data === 1) {
          ...
          try {
            ...
            if (postResponse.data === "插入成功") {
              new proxy.$tips('添加成功，并创建默认流程', 'success').message_();
              dialogFormVisible.value = false;
              await loadCompanyList();
            } else {
              new proxy.$tips('流程创建失败', 'warning').message_();
            }
          } catch (templateError) {
            console.error('流程模板操作失败:', templateError);
            new proxy.$tips('公司已添加，但流程初始化失败', 'error').message_();
          }
        } else if (res.data === 2) {
          new proxy.$tips('添加失败，用户名已存在', 'error').message_();
        } else {
          new proxy.$tips('未知错误，添加失败', 'error').message_();
        }
      } catch (e) {
        console.error('添加公司全过程失败:', e);
        new proxy.$tips(`操作失败: ${e.message || '服务器异常'}`, 'error').message_();
      } finally {
    ...
    const confirmDelete = (row) => {
      ...
      .then(() => {
        deleteRow(row.id,row.c_username);
      })
      .catch(() => {
        // 用户取消删除
      });
    ...
    // 删除记录
    const deleteRow = async (id,username) => {
      try {

        tableLoading.value = true;
        await axios.delete(`api/organization/delete/${id}`);
        await axios.delete(`api/bpmn-xml/deleteByNameAndUserName/main/${username}`)
