...
export default {
  components: { OfficeBuilding, Menu,User,Document,Box },
  ...
  setup() {
    ...
    const saveBreadcrumb = () => {
      localStorage.setItem('dep_breadcrumb', JSON.stringify(breadcrumbItems.value));
      // 同时保存当前路径，用于刷新页面时恢复
      localStorage.setItem('dep_currentPath', route.path);
    };
    ...
    const updateBreadcrumbFromRoute = () => {
      ...
      if (!result) {
        // 检查是否有保存的面包屑
        const savedBreadcrumb = localStorage.getItem('dep_breadcrumb');
        if (savedBreadcrumb) {
    ...
    const loadBreadcrumb = () => {
      const savedBreadcrumb = localStorage.getItem('dep_breadcrumb');
      const savedPath = localStorage.getItem('dep_currentPath');
    ...
    watch(
      ...
      (newPath) => {
        // 只有当路由真正变化时才更新面包屑
        const currentPath = localStorage.getItem('dep_currentPath');

        // 如果路径变化了，先保存当前路径
        if (newPath !== currentPath) {
          localStorage.setItem('dep_currentPath', newPath);
